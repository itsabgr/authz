...
// The AuthzFunc type is an adapter to allow the use of ordinary
// function as Authz mutator.
type AuthzFunc func(context.Context, *model.AuthzMutation) (model.Value, error)

// Mutate calls f(ctx, m).
func (f AuthzFunc) Mutate(ctx context.Context, m model.Mutation) (model.Value, error) {
	if mv, ok := m.(*model.AuthzMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *model.AuthzMutation", m)
}
